...
from topdown_parser.transition_systems.transition_system import Decision, TransitionSystem, DecisionBatch
...
#         return True
    ...
    #     selected_nodes = torch.argmax(mask * children_scores, dim=1)
    #     if self.pop_with_0:
    #         pop_mask = torch.eq(selected_nodes, 0)
    #     else:
    #         pop_mask = torch.eq(selected_nodes, state.stack.peek())
    #     push_mask: torch.Tensor = 1 - pop_mask
    #     edge_labels = torch.argmax(scores["edge_labels_scores"], 1)
    #     constants = torch.argmax(scores["constants_scores"], 1)
    #     lex_labels = torch.argmax(scores["lex_labels_scores"], 1)
    #
    #     return DecisionBatch(selected_nodes, push_mask, pop_mask, edge_labels, constants, lex_labels, state.constant_mask())
