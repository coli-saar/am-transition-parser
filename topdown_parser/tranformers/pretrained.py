...
# These classes are basically the same as the implementation in AllenNLP 1.0
# However, we compute a weighted average of the layers here.


@TokenEmbedder.register("scalar_mix_pretrained_transformer")
class ScalarMixPretrainedTransformerEmbedder(TokenEmbedder):
    """
    ...
    def __init__(
            ...
            train_scalar_mix : bool = True,
            initial_scalar_mix : Optional[List[float]] = None
    ) -> None:
        super().__init__()
        self.config = AutoConfig.from_pretrained(model_name, output_hidden_states=True)
        self.transformer_model = AutoModel.from_pretrained(model_name, config=self.config)

        self.train_scalar_mix = train_scalar_mix
        self.scalar_mix = ScalarMix(self.config.num_hidden_layers,
                                    trainable=train_scalar_mix, initial_scalar_parameters=initial_scalar_mix)
        if sub_module:
    ...
    def forward(
        ...
        with torch.set_grad_enabled(self.train_scalar_mix):
            embeddings = self.scalar_mix(embeddings[1:])
...
@TokenEmbedder.register("scalar_mix_pretrained_transformer_mismatched")
class ScalarMixPretrainedTransformerMismatchedEmbedder(TokenEmbedder):
    """
    ...
    def __init__(
            self, model_name: str, max_length: int = None,
            train_parameters: bool = False,
            train_scalar_mix : bool = True,
            initial_scalar_mix : Optional[List[float]] = None
    ) -> None:
        ...
        self._matched_embedder = ScalarMixPretrainedTransformerEmbedder(
            model_name, max_length, train_parameters=train_parameters, train_scalar_mix=train_scalar_mix,
            initial_scalar_mix=initial_scalar_mix
        )
